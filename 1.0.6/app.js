...
const presets = {
    ...
    // ===== 新增诗词 =====
    qibushi: "煮豆燃豆萁豆在釜中泣本是同根生相煎何太急",  // 七步诗
    wangtianmenshan: "天门中断楚江开碧水东流至此回两岸青山相对出孤帆一片日边来", // 望天门山
...
const DOMElements = {
    ...
    presetToggleIcon: document.getElementById('preset-toggle-icon'),
...
function bindEvents() {
    ...
    // 预设面板折叠切换
    DOMElements.presetToggle.addEventListener('click', togglePresetCollapse);

    // 诗词下拉选择
    DOMElements.poemSelect.addEventListener('change', function(e) {
        const value = this.value;
        if (value && presets[value]) {
            DOMElements.charInput.value = presets[value];
            // 关闭预设面板（可选）
            DOMElements.presetPanel.classList.add('hidden');
            // 重置下拉框，以便下次再次选择同一首
            this.value = '';
...
// ===== 预设面板折叠功能 =====
function togglePresetCollapse() {
    const grid = DOMElements.presetGrid;
    const icon = DOMElements.presetToggleIcon;
    const isCollapsed = grid.classList.toggle('collapsed');
    icon.textContent = isCollapsed ? '+' : '−';
    localStorage.setItem('presetCollapsed', isCollapsed ? 'true' : 'false');
}

function restorePresetCollapse() {
    const saved = localStorage.getItem('presetCollapsed');
    // 默认为折叠（true），即加载时隐藏
    const shouldCollapse = saved !== null ? saved === 'true' : true;
    if (shouldCollapse) {
        DOMElements.presetGrid.classList.add('collapsed');
        DOMElements.presetToggleIcon.textContent = '+';
    } else {
        DOMElements.presetToggleIcon.textContent = '−';
    }
}
