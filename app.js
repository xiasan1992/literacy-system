...
window.switchPage = switchPage; // 暴露给 HTML 里的 onclick 使用
...
function startQuiz() {
    ...
    originalQuizList = shuffleArray(chars);
...
function retryQuiz() {
    ...
    currentQuizList = shuffleArray([...originalQuizList]);
    currentIdx = 0;
    quizResult = { correct: 0, wrong: 0 };
    isReviewMode = false;
    switchPage('quiz');
...
function startReview() {
    ...
    if (reviewList.length === 0) {
        ...
        originalQuizList = []; // 复习模式不支持直接 retry 完整的原始列表
...
function isMastered(char) {
    ...
    // 如果曾经答错过，但是当前连续答对达到了3次 -> 掌握
    if (d.streak >= 3) return true;
    
    // 兼容旧数据：如果没有 streak 字段，但是正确次数远大于错误次数（暂定为正净胜>=3即可）
